...
Commit implementasi, hasil build check, source digest dan preflight akan direkam dalam `mainnet-followup-2026-09-15/release-validation.json`. Preflight default mempertahankan requirement contoh sebagai pending; evidence lokal tidak otomatis mengubahnya menjadi signoff. Archive review dibuat dari final commit yang bersih dengan `npm run review:package`, dan manifest di samping archive mencatat commit serta SHA-256 file sebenarnya.
