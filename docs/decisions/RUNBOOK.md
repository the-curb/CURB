...
2. **Stop what must stop** under [the operator policy](OPERATIONS.md): any signer proposes a mint stop; the configured operator quorum executes it. A component's claims stop only under the policy's reasons and quorum. CompanySeries has no separate one-signer guardian.
...
| A stop is needed and the operator is a quorum | Nothing until the quorum executes; minting continues meanwhile if other checks allow it | One signer proposes at once; a second confirms for a 2-of-3 Safe. Record proposal time separately from the executed stop. If quorum is unavailable, escalate and show that the pause is pending; no single signer can force it | "A stop was requested at *t1*; minting stopped only when the operator transaction executed at block *b*, time *t2*, reason *r*." |
...
| The store is unreachable | Nothing on chain changes. A paid call that cannot be admitted fails without a debit; a debit whose commit acknowledgement was lost may report `charged: UNKNOWN` | `/api/state` says STORE_UNREADABLE with the reason. Preserve request/settlement identifiers and logs; restore access, reconcile an uncertain debit by its existing identifier, and never create a second debit to resolve uncertainty. Then resume indexing from the preserved cursor | "The desk's record could not be read since *t*. Your request's charge is [NO / UNKNOWN / confirmed amount], as recorded. An UNKNOWN result is being reconciled; it is not a promise that the request was free. The chain is unaffected." |
...
The conditions, alert delivery and series status blocks exist. `contracts/evidence/drill-local.json` records the six local position scenarios described above with mock components; it does not establish public operator readiness or all credits procedures. The Robinhood Chain treasury creation is recorded separately. No approved public CompanySeries or CreditDesk deployment is established by these local records; current production incidents require their own dated evidence.
