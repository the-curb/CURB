...
- The operator of a series is one address, set at construction. The mainnet preparation source changes rotation to two steps: the current operator nominates with `transferOperator(next)`, and only that nominee can complete it with `acceptOperator()`. The current operator retains authority until acceptance and can cancel with `cancelOperatorTransfer()`. For a pilot it is proposed to be a **multisig of at least three signers with a quorum of two**, none of whom is a signer for any issuer or custodian of the components, with the signer list published on this site. This local change still requires independent review and a pinned release; it is not evidence of public deployment.
...
| Stop minting | `setMintPaused(true, reason)` | immediately on: a DARK condition on a component (drift of code, implementation, admin or asset; a shortfall), a reverted mint that the ledger cannot explain, or an issuer notice | any signer proposes; **quorum executes** with the current contract | on chain with the reason; incident opened |
...
| Nominate the next operator | `transferOperator(next)` | the multisig address must change and the next Safe is verified on the series' chain | current quorum | `OperatorTransferProposed`; current operator retains authority |
| Accept the operator role | `acceptOperator()` | nominee reviewed the series and verified its own ability to operate it | nominee's quorum | `OperatorChanged`; pending nomination is cleared |
| Cancel a nomination | `cancelOperatorTransfer()` | wrong recipient, abandoned rotation or superseded plan | current quorum | `OperatorTransferCancelled`; publish the reason in the rotation log |

## The treasury of the credit desk

[The token record](TOKEN.md) is decided; the credit desk's treasury — the address every top-up goes to, an immutable of the desk that the site verifies against the record on every tick — is **the designated Safe on Robinhood Chain** recorded below. Its spending is under this policy. The Ethereum series operator requires separate chain-specific verification even if the intended signers are the same:
...
**Created 13 September 2026.** The operator's Safe — 2-of-3, Safe 1.4.1 with the L2 singleton — exists on Robinhood Chain at `0x4E69723F9Ba9fA2C9842d77b240b2C0F601ac219` (transaction `0x8f74a9ad70628a592ee624628215cd788a2f08d9b3d2b115059c015705314268`, block 61,691,826; the record with the plan, the receipt and the read-back is `contracts/evidence/safes/safe.4663.json`). Its owners, as the product owner named them that day and as the chain answers `getOwners()` — **thecurb 1** `0xC33DAfAC1c65a92e956519Aa73E29a0b8c39214B`, **thecurb 2** `0x5657dF9f072C536FcdE00BE2eF86c2240E4f8a53`, **thecurb 3** `0x5cc762A11b05F9cCdaE3839C56F5Beb14C40F15D` — threshold 2, as `getThreshold()` answers. Verified independently through the public node and on the explorer. The names are the product owner's designations of 13 September 2026; the product owner's confirmation that none of the three signs for an issuer or custodian of the components is recorded with them. This Safe is the designated Robinhood Chain credit-desk treasury. The candidate series is on Ethereum: the operator address, deployed Safe code, owners and threshold must be verified on that chain before it can be used there. The Robinhood record alone establishes none of those Ethereum facts.
...
A signer is replaced using the Safe's owner-management procedure while preserving quorum and proving the new signer can co-sign. If the multisig address stays the same, the series operator address does not change. If it changes, first verify the new Safe on the series' chain and publish the plan; the old quorum nominates it with `transferOperator(next)`, then the new Safe executes `acceptOperator()` through its own quorum. Verify `operator()` and cleared `pendingOperator()` and record both transactions. A nomination does not complete rotation; the old operator can replace or cancel it while pending. A lost signer key can be rotated while quorum holds. If quorum cannot be reached, the current operator cannot nominate or execute new actions; existing holder permits and contract rules continue to apply. Two-step transfer prevents an unaccepted nomination from immediately losing authority; it cannot recover already-lost quorum or holder keys.

## Decision logging

Every operator transaction has an entry in the incident, rotation or access log with: date, signers, condition/request, evidence reviewed, expected effect, chain, Safe nonce and executed transaction hash. Stop/resume calls also carry a reason string on chain; permit and rotation events do not automatically include that prose, so their written reasons belong in the log. Publish an appropriate redacted log; keep participant personal data out of the public repository. The [mainnet dossier](../mainnet/PREPARATION.md#operating-readiness) provides unassigned owner and signoff fields.
