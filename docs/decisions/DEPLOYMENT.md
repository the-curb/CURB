...
0. **The multisig.** `node scripts/plan-safe.ts <owner> <owner> <owner> --threshold 2` plans a 2-of-3 Safe on Robinhood Chain, unsigned (the [operator policy](OPERATIONS.md)); a funded wallet sends it; the owners and the threshold are verified on the explorer. Its address is the treasury below and a series' operator. It acts without a hosted interface: `node scripts/safe-tx.ts --safe … --to … --data …` prints the hash the owners approve (checked against the Safe's own), the `approveHash` each sends from their wallet, and the `execTransaction` anyone sends once the quorum has approved — rehearsed on a local chain with Safe's code as Robinhood Chain has it.
...
5. **The first top-up.** Before the desk is announced, the operator tops up a key of their own with a small amount and reads it back on the services page. The desk refuses a token that pays the treasury less than the amount (`DeltaWrong`), so a fee-on-transfer token shows here as a revert and nothing else; the desk is not announced until a top-up lands.
6. **The pool.** When the token trades in a pool the reader knows — a constant-product pair (`uniswap-v2-pair`) or a concentrated-liquidity pool (`uniswap-v3-pool`) — its address goes into the record's `priceSource` from the chain, with the quote taken as dollars (USDG) or priced by a feed (ETH / USD for WETH), and the same line is set again. Until then the price list is in dollars and nothing is quoted. The public node keeps about ten minutes of state, so a top-up's own block is priced from the pool's events; nothing about that needs configuring.
