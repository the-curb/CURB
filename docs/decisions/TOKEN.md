...
**Execution calculation selected, 15 September 2026:** allocate the first US$40,000 to review and the next US$15,000 to legal. Apply 40%/20%/20%/20% only to the amount remaining after those priorities. Round review, legal and infrastructure down to cents; put the rounding remainder in reserve. `npm run mainnet:budget` implements this without floating-point arithmetic. This clarifies the earlier priorities and percentages; each cent is allocated once. At low proceeds, infrastructure is unfunded and requires a separate identified budget before service commitments.

Only freely available, separately documented treasury receipts enter this calculation. Trading volume, market capitalization, bonding-curve reserves and locked pool liquidity are excluded. Credit top-ups are tracked separately against service obligations. Current [PONS research](../mainnet/EXTERNAL-FACTS-2026-09-15.md) describes curve reserves becoming liquidity and creator income coming from fee distributions; it does not establish funds available to The Curb. Expected token allocation/vesting and launchpad charges must be disclosed from reviewed terms before a sale, then matched against chain evidence. The default planning input is zero available proceeds and unmeasured operating costs.
...
- **Prelaunch review funding and launchpad verification.** The executable residual formula is selected for planning; actual funding and launch access remain unverified. PONS v2 uses a curve then v4, both unsupported by the current price reader. No conversion is enabled by relabelling either as a v2/v3 pool. See the dated [external facts](../mainnet/EXTERNAL-FACTS-2026-09-15.md).
