...
**Status:** Proposed, 12 September 2026. Not decided. Awaits the product owner. The mechanism's §16 says the only sensible function to test for CURB is payment for data and integration services that actually exist, with stated prices, a stated conversion, slippage limits, credit validity and a cancellation policy. This record proposes each of those, and the order in which they happen: the services and the gate first, the token after.
...
| --- | --- | --- |
| Opening a key | The minimum paid in before a key can be used, cumulative across top-ups | US$20.00 |
| Evidence versions | Every archived version of one source's record — the identities, the dates, the parsed record of each — for a series | US$0.05 per call |
| Journal by day | The product's verified changes on one UTC day, as the Gazette prints them | US$0.05 per call |
| Alert delivery | The desk's conditions — raised, cleared, still active — posted to a webhook the key registered, once per change | US$0.10 per delivery |
...
3. **The token launches.** The address, decimals and supply are read from the chain and recorded; the desk is deployed pointing at that address and the operator multisig as treasury; the pool is recorded when it exists; `CURB_TOKEN_ADDRESS`, `CURB_CREDIT_DESK`, `CURB_PRICE_SOURCE` and `CURB_CREDIT_NETWORK` are set from those records and nothing else.
...
`contracts/src/CreditDesk.sol` and its fifteen tests; `lib/credits/` — the price list, the rate reader, the key store, the top-up indexer, the paid-endpoint guard and the webhook subscriptions; `/services` and `/api/credits`; the rehearsal on a local chain. No token exists; no pool exists; nothing is configured in production; every figure on the services page that depends on a rate says so.

## Open, for the person who decides

- The prices above, and the US$20 minimum.
