...
**Execution calculation, proposed 15 September 2026 and confirmed by the product owner on 17 September 2026 as the reading of the split above (a clarification, not a change — no notice period starts):** allocate the first US$40,000 to review and the next US$15,000 to legal. Apply 40%/20%/20%/20% only to the amount remaining after those priorities. Round review, legal and infrastructure down to cents; put the rounding remainder in reserve. `npm run mainnet:budget` implements this without floating-point arithmetic. It reads the priorities and percentages decided on 12 September so that each cent is allocated once; a different split from here on is a change of a decided term, with the thirty days' notice. At low proceeds, infrastructure is unfunded and requires a separate identified budget before service commitments.
