...
| On-chain verification of every address the evidence names — code, symbol, decimals, `asset()` against the claimed raw token, and the EIP-1967 slots behind a proxy (an upgrade the code hash cannot see) | Implemented, running on the tick | the series page, `/api/positions/apple-s1/evidence` |
...
| Product API (§10) with string amounts, previews that send nothing, and an indicative value (§8) for A from dated sources — the wrapper's conversion at a recorded block and the desk's AAPL / USD sample — with B not available and the lot never totalled | Implemented | `/api/positions`, `/api/wallets/…`, `/api/status` |
...
| Fork tests of the real component A on Ethereum (C08, G3): identity, transfer, a series round trip, unwrap (T21), the raw token's derived balance, the wrapper's size, and who stands behind each address (EIP-1967 implementation and admin, `owner()`, `paused()`) — findings written to `contracts/evidence/apple-s1.fork.json` and shown dated | Implemented; block not pinned (public node) | `contracts/test/fork/`, the series page |
