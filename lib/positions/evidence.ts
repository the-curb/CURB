...
export type EvidenceKind = 'xstocks-asset' | 'ondo-addresses';
...
export const EVIDENCE_SOURCES: readonly EvidenceSource[] = [
  { id: 'xstocks:AAPLx', kind: 'xstocks-asset', seriesId: APPLE_S1.id, component: 'A', url: XSTOCKS_ASSET_URL('AAPLx'), title: 'xStocks public asset record for AAPLx' },
  { id: 'ondo:AAPLon', kind: 'ondo-addresses', seriesId: APPLE_S1.id, component: 'B', url: ONDO_ADDRESSES_URL('AAPLon'), title: 'Ondo contract addresses for AAPLon' },
];
...
function parseFor(kind: EvidenceKind, fetched: Fetched): { parse: ParseStatus; parsed: Observation['parsed']; status: FetchStatus; detail: string | null } {
  if (fetched.status !== 'OK' || fetched.raw === null) return { parse: 'NOT_PARSED', parsed: null, status: fetched.status, detail: fetched.detail };
  const result = kind === 'xstocks-asset' ? parseXstocksAsset(fetched.raw) : parseOndoAddresses(fetched.raw);
  if (result.ok) return { parse: 'PARSED', parsed: result.asset, status: 'OK', detail: null };
  return { parse: 'NOT_PARSED', parsed: null, status: result.status, detail: result.detail };
}
...
export async function archiveEvidence(store: Store, now: Date, sources: readonly EvidenceSource[] = EVIDENCE_SOURCES): Promise<ArchiveOutcome[]> {
  ...
  for (const source of sources) {
    ...
    const records: SnapshotRecord[] = [{ key: evidenceLatestKey(source.id), observedAt: observation.readAt, payload: observation as unknown as Record<string, unknown> }];
    if (version === 'NEW' && observation.hash !== null) {
      records.push({ key: evidenceVersionKey(source.id, observation.hash), observedAt: observation.readAt, payload: observation as unknown as Record<string, unknown> });
    }
    const written = await store.writeSnapshots(records);
    outcomes.push({ sourceId: source.id, status: observation.status, parse: observation.parse, hash: observation.hash, version, recorded: written.state === 'WRITTEN' });
  }
  return outcomes;
}

/** The latest observation per source, as the store has it. */
export async function latestEvidence(store: Store, sources: readonly EvidenceSource[] = EVIDENCE_SOURCES): Promise<{ source: EvidenceSource; observation: Observation | null; storeFault: string | null }[]> {
  const read = await store.snapshots(EVIDENCE_PREFIX);
  return sources.map((source) => {
    if (read.state === 'UNREAD') return { source, observation: null, storeFault: `${read.reason}${read.detail ? ` — ${read.detail}` : ''}` };
    return { source, observation: observationOf(read.value.find((s) => s.key === evidenceLatestKey(source.id))), storeFault: null };
  });
}
