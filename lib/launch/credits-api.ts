import type { CreditsStatus } from '../credits/config.ts';
import { latestRun } from '../credits/maintenance.ts';
import { topUpReadiness } from '../credits/top-up.ts';
import { receipts } from '../credits/receipts.ts';
import { MINIMUM_DECISION, MINIMUM_OPEN_CENTS, NOTICE_DAYS, PRICES_DECISION, PRICES_STATUS, SERVICES, TERMS_DECISION, TERMS_STATUS, centsText } from '../credits/prices.ts';
import { curbForCents, curbText, usd18Text } from '../credits/rate.ts';
import type { Store } from '../store/types.ts';
import { LAUNCH_READ_MAX_AGE_MS } from './evidence.ts';
 ...
 * but a recent read bound to the current configuration, using the same
 * checks as the services page. The raw observations remain visible even
 * when they cannot support a quote. Dependencies are explicit so tests
 * exercise this exact HTTP response without opening the configured store.
 */
export async function creditsResponse(request: Request, status: CreditsStatus, store: Store, now = new Date()): Promise<Response> {
  const configured = status.state === 'CONFIGURED';
  const [readiness, run, paid] = await Promise.all([topUpReadiness(store, status, now), configured ? latestRun(store) : null, receipts(store)]);
  const { rateRead, code } = readiness;
  const verified = readiness.quoteReadiness.codeCurrent;
  const rate = rateRead?.rate ?? null;
  const rateFault = rateRead?.storeFault ?? null;
  const rateCurrent = readiness.quoteReadiness.rateCurrent;
  ...
  if (usdParam !== null) {
    ...
    if (m === null) quote = { error: 'USD_MALFORMED', detail: 'usd is a dollar figure with at most two decimals, e.g. 20 or 20.50' };
      ...
      if (cents === 0n) quote = { error: 'USD_ZERO', detail: 'a quote for nothing is nothing; the desk refuses a top-up of zero' };
      ...
      else if (!verified) quote = { usdCents: cents.toString(), usd: centsText(cents), curb: null, state: 'HELD', detail: `the desk has no matching code verification for the current chain, address, token and treasury within the last ${LAUNCH_READ_MAX_AGE_MS / 60_000} minutes; nothing is quoted and top-ups are not credited from it — do not send one; wait for the next successful verification` };
        ...
        quote = { usdCents: cents.toString(), usd: centsText(cents), curb: null, state: 'HELD', detail: `the last rate is not a valid read for the current token, pool and quote source within the last ${LAUNCH_READ_MAX_AGE_MS / 60_000} minutes; nothing is quoted — wait for the next successful rate read` };
      } else {
        ...
        quote = {
          ...
          readAt: rate.at,
          state: 'QUOTED',
          note: 'read at that block; credit uses the mined-block rate or the lower lookback rate. The 5% buffer is an estimate, not a slippage limit or minimum credit guarantee; any excess stays on the key',
        };
      }
    }
  }
  ...
  return Response.json(
    {
      ...
      /** Quote eligibility is separate from configuration and the raw observations below; neither is an availability guarantee. */
