...
export async function creditsResponse(request: Request, status: CreditsStatus, store: Store, now = new Date()): Promise<Response> {
  ...
  if (usdParam !== null) {
    ...
    if (m === null) quote = { error: 'USD_MALFORMED', detail: 'usd is a dollar figure with at most two decimals, e.g. 20 or 20.50' };
      ...
      else if (!verified) quote = { usdCents: cents.toString(), usd: centsText(cents), curb: null, state: 'HELD', detail: `the desk has no matching code verification for the current chain, address, token and treasury within the last ${LAUNCH_READ_MAX_AGE_MS / 60_000} minutes; nothing is quoted — wait for the next successful verification` };
