...
2. Variables on the app service (**Variables → Raw editor** takes the table
   ...
   | `CURB_RPC_ROUTE` | optional — unset, the split above, so a paid balance is spent only on what the public node cannot serve (decided 21 September 2026, when the dRPC balance ran low). `all` sends every read to `CURB_RPC_URL` first, as before |
